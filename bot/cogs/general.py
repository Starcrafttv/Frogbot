...
class General(commands.Cog):
    ...
    async def invite(self, ctx, *, args=''):
        embed = Embed(
            title='\u200b',
            description=f'[Invite me](https://discord.com/oauth2/authorize?client_id={self.bot.client.user.id}&scope=bot&permissions=8)',
        ...
        if args[0] in ['prefix']:
                ...
                try:
                    ...
                    if 0 <= volume <= 100:
                        ...
                        if ctx.guild.id in self.bot.voiceClients:
                            self.bot.voiceClients[ctx.guild.id].volume = volume
            ...
            if len(args) > 1 and args[1]:
                try:
                    ...
                    if 0 <= timeout <= 6000:
                        ...
                        if ctx.guild.id in self.bot.voiceClients:
                            self.bot.voiceClients[ctx.guild.id].timeoutMax = timeout
                        await ctx.message.add_reaction('🐸')
                    else:
            ...
            if len(args) > 1 and args[1]:
                try:
                    ...
                    if ctx.guild.id in self.bot.voiceClients:
                        self.bot.voiceClients[ctx.guild.id].role = musicrole
                    await ctx.message.add_reaction('🐸')
                except Exception:
            ...
            if len(args) > 1 and args[1]:
                try:
                    ...
                    if ctx.guild.id in self.bot.voiceClients:
                        self.bot.voiceClients[ctx.guild.id].SpamChannelId = musicChannelId
                    await ctx.message.add_reaction('🐸')
                except Exception:
            ...
            if len(args) > 1 and args[1] == 'true':
                with self.bot.conn:
                    ...
                    self.bot.c.execute(f"UPDATE guilds SET Volume = 30 WHERE GuildID = '{ctx.guild.id}'")
                    self.bot.c.execute(f"UPDATE guilds SET MusicChannelId = 0 WHERE GuildID = '{ctx.guild.id}'")
                    self.bot.c.execute(f"UPDATE guilds SET Timeout = 600 WHERE GuildID = '{ctx.guild.id}'")
