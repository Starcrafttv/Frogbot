...
class Dmsystem(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if not message.guild and not message.author.bot and message.content[0] != '!':
            ...
            if not channelID:
                channel = await self.bot.client.fetch_channel(self.bot.supportLogChannelId)
                newChannel = await channel.guild.create_text_channel(f'ticket-{message.author.name}-{message.author.id}')
                category = self.bot.client.get_channel(self.bot.supportCategoryId)
                ...
                with self.bot.conn:
                    self.bot.c.execute(
                        f"INSERT INTO dms (UserID, ChannelID) VALUES ('{message.author.name}', '{newChannel.id}')")
                await message.reply("Thank you for your message! Our mod team will reply to you as soon as possible.", mention_author=False)
            else:
                channel = await self.bot.client.fetch_channel(channelID[0])
        ...
        if userID and message.content[0] != prefix:
            await self.bot.client.get_user(userID).send(message.content)
    ...
    async def close(self, ctx):
        ...
        if userID:
            with self.bot.conn:
                self.bot.c.execute(f"DELETE FROM dms WHERE PlaylistID = '{userID[0]}'")
