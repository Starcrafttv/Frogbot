...
def get_stats(stats):
    try:
        ...
        ax.bar([i + 0.2 for i in x], afk, width=0.4, alpha=0.8, align='center', color='orange', zorder=3, label='Afk')
        ...
        plt.savefig('bot/data/temp/stats.png', dpi=300, facecolor=fig.get_facecolor())
        return True
    except Exception:
...
# def __new_guilds(days, new):
#     try:
#         # Function to display how many guild my bot joins per day
#         n = (len(days) + 10) // 10
#         x = [i for i in range(len(days))]
#         plt.clf()
#         fig, ax = plt.subplots(1, 1)
...
#         ax.spines['right'].set_color('none')
#         ax.spines['left'].set_color('none')
#         ax.bar(x, new, width=0.4, alpha=0.8, align='center', color='#7289DA', zorder=3)
#         plt.grid(color='#2C2F33', linestyle=':', linewidth=1, axis='y', zorder=1)
#         ax.spines['left'].set_smart_bounds(True)
#         ax.spines['bottom'].set_smart_bounds(True)
#         plt.xticks(x, days, rotation=40, ha='right')
...
#         ax.tick_params(axis='x', which='major', labelsize=12)
#         ax.tick_params(axis='y', which='major', labelsize=12)
#         ax.set_ylabel('Guilds joined', fontsize=12)
#         ax.set_xlabel('')
#         plt.title('Guilds joined per day', fontsize=15)
#         plt.tight_layout()
#         plt.savefig('bot/data/temp/stats.png', dpi=300, facecolor=fig.get_facecolor())
#         return True
#     except Exception:
